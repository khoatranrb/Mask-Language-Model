...
class FFN(nn.Module):
    ...
    def __init__(self, num_hidden):
        ...
        self.w_1 = Step(Conv(num_hidden, num_hidden * 4, kernel_size=1, w_init='relu'))
        self.w_2 = Step(Conv(num_hidden * 4, num_hidden, kernel_size=1))
        self.dropout = nn.Dropout(p=0.1)
        self.layer_norm = nn.LayerNorm(num_hidden)

    def forward(self, input_, update=False):
        # FFN Network
        x = input_.transpose(1, 2)
        x = self.w_2(t.relu(self.w_1(x, update)), update)
        x = x.transpose(1, 2)
...
class EncoderPrenet(nn.Module):
    ...
    def __init__(self, embedding_size, channels):
        ...
        for i in range(hp.enc_conv1d_layers):
            conv1d = Step(Conv(in_channels=in_channels, out_channels=out_channels, kernel_size=kernel_size,
    ...
    def forward(self, input, update=False):
        """
        :param input: B*T*d
        :return:
        """
        ...
        for conv1d, bn, dropout in zip(self.conv1d_layers, self.bn_layers, self.dropout_layers):
            input = dropout(t.relu(bn(conv1d(input, update))))     # B*d*T
